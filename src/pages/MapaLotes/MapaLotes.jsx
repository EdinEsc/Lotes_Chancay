  ...
  useEffect(() => {
    ...
    const initMap = () => {
      ...
      try {
        ...
        // terreno.features.slice(1).forEach((feature, index) => {
        //   L.geoJSON(feature, {
        //     color: "#FFFFFF",
        //     weight: 3,
        //     opacity: 0.9,
...
terreno.features.slice(1).forEach((feature, index) => {
  L.geoJSON(feature, {
    color: mapConfig.COLOR_SECUNDARIO, // Usa el color secundario (verde por defecto)
    weight: 3,
    opacity: 0.9,
    // dashArray: index % 2 === 0 ? "5, 5" : "8, 3",
    dashArray: "5, 5",
        ...
        filteredLots.forEach(lot => {
          ...
          const lotMarker = L.marker([lat, lng], {
            icon: L.divIcon({
              className: `lot-marker ${lot.status.toLowerCase()} ${isLargeLot ? 'large-lot' : ''}`,
              html: `
                <div class="marker" style="
                  position: relative;
                  width: ${isLargeLot ? mapConfig.TAMAÑO_MARCADOR_GRANDE + 4 + 'px' : mapConfig.TAMAÑO_MARCADOR_NORMAL + 'px'}; 
                  height: ${isLargeLot ? mapConfig.TAMAÑO_MARCADOR_GRANDE + 4 + 'px' : mapConfig.TAMAÑO_MARCADOR_NORMAL + 'px'}; 
                  background: ${lot.status === 'Disponible' ? 
                    `radial-gradient(circle at 30% 30%, ${mapConfig.COLOR_SECUNDARIO}, #2E7D32)` : 
                    lot.status === 'Reservado' ?
                    'radial-gradient(circle at 30% 30%, #FF9800, #EF6C00)' :
                    'radial-gradient(circle at 30% 30%, #9E9E9E, #757575)'}; 
                ...
                ">
                  ...
                  <div style="
                    ...
                    border-top: 8px solid ${lot.status === 'Disponible' ? '#2E7D32' : lot.status === 'Reservado' ? '#EF6C00' : '#757575'};
                  "></div>
                </div>
              `,
              iconSize: isLargeLot ? [38, 48] : [35, 45],
              iconAnchor: isLargeLot ? [19, 48] : [17, 45]
            })
          }).addTo(map);
          ...
          lotMarker.bindTooltip(`
            <div class="tooltip" style="
              ...
              padding: 10px; 
              border-radius: 10px; 
              border: 3px solid ${lot.status === 'Disponible' ? mapConfig.COLOR_SECUNDARIO : lot.status === 'Reservado' ? '#FF9800' : '#9E9E9E'};
              box-shadow: 0 6px 25px rgba(0,0,0,0.4);
              backdrop-filter: blur(12px);
              max-width: 200px;
            ">
        ...
        <div className="w-full max-w-7xl mx-auto px-5 py-10 bg-white flex-1 flex flex-col">
          ...
          <div className="map-container-wrapper relative z-0 bg-white">
            <div className="relative w-full 
              ...
              <LotInfoPanel
                showLotInfo={showLotInfo}
                setShowLotInfo={closeLotInfoPanel}
                selectedLot={selectedLot}
                mapConfig={mapConfig}
              />
